...
## 2026-07-10 — Lead-email webhook outage found + fixed (John's "25 calls, no leads")

**Trigger:** John's WhatsApp (~2026-07-08 23:06): "we had 25 calls from twilio to number but no leads". Investigation findings:

1. **The "25 calls" are the Twilio call log for `+18882934492` over ~5 weeks (34 since Jun 1)** — mostly OUR OWN test calls (José +34, John +1312, Pedro's DID and other 726-team numbers). Only ~6 external-caller calls since Jun 1, and everything before the notifier existed (2026-06-27) could never have emailed anyone. Same class of misunderstanding as the 2026-05-18 "32 missed weekend calls" episode.
2. **BUT a real bug was found:** the notifier webhook was down since ~2026-07-06 — VAPI was getting 401s because the BR number's `server.secret` no longer matched prod `CRON_SECRET` (most likely re-set wrong during the 2026-07-06 cleanup re-run of `set-br-lead-webhook.js`; note its success log line says `secret=set` but only actually checks `server.url` — cosmetic bug still there). Endpoint itself was always healthy (401 on bad auth, dry-run works, Vercel envs correct).
3. **Impact: exactly ONE lost lead** — 2026-07-09 14:26 UTC call `019f4745-e5e4` (261s, caller +13057135164 Miami, "Ricardo", marbella@gmail.com, New Construction $7M Los Angeles, hung up mid-flow before quote). Likely a demo call by Javier/Palmstudyo (Sonant project), NOT a real John lead — confirm before re-sending to leads@. Re-send command: `GET /api/vapi-lead-email?callId=019f4745-e5e4-7559-8f90-460b61fba946` (Bearer CRON_SECRET). The 2026-06-28 calls (65s/58s) never reached Jennifer with data → correctly skipped.
4. **Fixes shipped:** (a) re-ran `set-br-lead-webhook.js` with the correct CRON_SECRET (webhook re-armed on `+18882934492` only, squadId intact); (b) per José: `PARTIAL_RECIPIENTS` now ALSO includes `leads@farmerbrown.com` (was José-only) — both audiences now José + leads@; ping build marker bumped to `h9-partials-to-leads`.
5. **VERIFY NEXT:** make a short test call to `+18882934492`, give Jennifer name+email, confirm the `[In progress]` partial arrives at BOTH inboxes, then the complete email at end of call.
6. **Gotcha discovered:** VAPI call-history retention on our plan is **14 days** — `/call` with older `createdAtGe` errors out. Twilio (John's account, creds in `.env`) is the long-horizon call log; VAPI is only good for the last 2 weeks.
...
**0b. Repo + VAPI cleanup.** Archived to `archive/`: Rebecca/Bryan/bonds + binding scripts/agents/docs (scrapped). **VAPI deleted:** Test Squad (Rebecca/Bryan) + legacy BR Sales EN squad; 8 assistants (Test Dispatcher, Bryan, Rebecca, old BR-Net Sarah, bak-farmerbrown-sarah, farmerbrown-es-wip, Riley, legacy Grace BR Sales); 2 tools (end_call_tool, transfer_to_bonds_specialist). Org now **20 assistants / 14 tools / 6 squads / 5 numbers**. **Fixed:** QA number `+17027108075` was misrouted to the dead Test Squad → repointed to BR Unified. **Resolved 2026-07-06:** `+13412049248 → farmerbrown-en` was an old March prototype ("Brian", 0 tools, unused since March) — assistant **DELETED**, full config archived to `archive/agents/farmerbrown-en-brian/`, number **PARKED** (unassigned, reserved, not released). `set-br-lead-webhook.js` now pins to `+18882934492` only. **CLAUDE.md is now STALE** (references deleted assets) — refresh when building the monorepo master CLAUDE.md.

**0c. Monorepo restructure (DONE, pushed).** `farmerbrown` is now a **two-folder monorepo**: `voice-agents/` (this project) + `billing/` (imported via `git subtree --squash` — full history stays in the standalone `farmerbrown-billing` repo, which becomes the history archive). Master `CLAUDE.md` at the repo root points to both. Branch `chore/repo-cleanup` was merged to `main` (fast-forward); backup tag `backup/pre-monorepo-2026-07-06`. **PENDING (joint w/ José): repoint Vercel** — in the billing Vercel project, set Git repo → `jaromeroes/farmerbrown` and **Root Directory = `billing/`**, redeploy, verify portal + `/api/vapi-lead-email`, THEN retire the old `farmerbrown-billing` repo. Until then the live deploy still runs from the OLD repo (unaffected by the monorepo). **Follow-up:** this file + `voice-agents/CLAUDE.md` still name deleted VAPI assets (Rebecca/Bryan/bonds/legacy Grace BR Sales) — a deep content refresh was done 2026-07-06 (Rebecca section removed, Grace BR Sales delete noted, margin→35%, premium formula marked superseded).
