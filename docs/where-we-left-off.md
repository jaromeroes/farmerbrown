...
**Last touched:** 2026-05-08 PM — Grace v1.18 + Rachel v2.4 + new BR Direct-Dial Proxy v1.0 + new H&A team fallback line. **Three significant bugs surfaced and were fixed during this session** (see "Bugs fixed today" section below). Grace is now answering the line correctly (verified by test call). Direct-dial flow for "Pedro" is wired but **not yet end-to-end verified by test call** — last attempt happened during the broken-squad window, the proxy has since been recreated with a new ID and re-attached to the squad.
...
| **Grace** (BR receptionist Unified) | **v1.18** (NEW) | v1.16 fixes + **directory ambiguities resolved** (George → Jorge; Maria → María Portillo, both in INTERNAL DIRECTORY) + **direct-dial pivoted from PBX/DTMF to per-person DIDs** + **first wired entry is Pedro** (Gustavo flipped back to `pending`, he's in Medellín / asleep). Step P1 branches on a `Direct-dial?` column: `yes` → squad destination `BR Direct-Dial Proxy` → tool `transfer_to_specific_person` with the matching DID (raw E.164, no extension); `pending` → live-agent line as v1.16. Internal extensions (148, 124, 181…) remain in the directory as info for live-agent rep transcripts but are no longer used by VAPI for routing. **Existing-quote rama still disconnected** (waiting on a separate dedicated number from Pedro). |
...
2. **First test priority — direct-dial verification (Pedro).** Call `+18882934492`, ask for "Pedro". Listen for:
   - Grace says *"Of course — connecting you to Pedro. One moment."*
   - Brief silence (proxy taking over).
   - The call connects directly to Pedro's phone (`+17262334655` rings him).
   - **If it works:** ready to wire more directory entries — collect each person's DID, add them as destinations to `transfer_to_specific_person`, flip their `Direct-dial?` to `yes` in Grace's directory, bump version, redeploy.
   - **If it fails (wrong destination / no audio / busy / dropped):** check the `endedReason` in the VAPI call log first. Then verify the DID is correct (Pedro's actual phone) and that VAPI can dial US toll/non-toll numbers from the configured account.
