...
**Last touched:** 2026-05-08 PM — Grace v1.19 + Rachel v2.4 + BR Direct-Dial Proxy v1.0 + H&A team fallback line. **Direct-dial verified working end-to-end:** José called +18882934492, asked for Pedro, the call connected to Pedro's phone (+17262334655). RingCentral phone-list export imported as `docs/farmer-brown-phone-directory.md` — 21 shortlist entries, 65 other employees, 12 inventory numbers. Pedro's last name (Neumann) and extension (275) added to Grace's directory in v1.19; Grace now says *"Of course — connecting you to Pedro Neumann. One moment."* before transferring.
...
| **Grace** (BR receptionist Unified) | **v1.19** (NEW) | v1.16 fixes + **directory ambiguities resolved** (George → Jorge; Maria → María Portillo, both in INTERNAL DIRECTORY) + **direct-dial pivoted from PBX/DTMF to per-person DIDs** + **Pedro Neumann wired and verified end-to-end** (Gustavo flipped back to `pending`, he's in Medellín / asleep). Step P1 branches on a `Direct-dial?` column: `yes` → squad destination `BR Direct-Dial Proxy` → tool `transfer_to_specific_person` with the matching DID (raw E.164, no extension); `pending` → live-agent line as v1.16. v1.19 added Pedro's last name (Neumann) and extension (275) from the RingCentral export so Grace says the full name when transferring. Internal extensions (148, 124, 181…) remain in the directory as info for live-agent rep transcripts but are no longer used by VAPI for routing. **Existing-quote rama still disconnected** (waiting on a separate dedicated number from Pedro). |
...
2. **Direct-dial — VERIFIED for Pedro Neumann (2026-05-08 PM).** Test call landed at Pedro's phone successfully. To scale to the rest of the shortlist:
   - DIDs are already in `docs/farmer-brown-phone-directory.md` (RingCentral export 2026-05-08). Copy them into the `DESTINATIONS` array of `scripts/create-tool-transfer-to-specific-person.js`.
   - Use the Softphone DID where multiple lines exist for the same person (e.g. Pedro Neumann had both Softphone `(726) 233-4655` and Desk Phone `(726) 203-8815` — Softphone is the wired one).
   - Re-run the create-tool script (idempotent — updates destinations[] in place).
   - Flip `Direct-dial?` from `pending` → `yes` for each newly-wired person in Grace's INTERNAL DIRECTORY (Step P1 of `agents/receptionist-buildersrisk-unified/system-prompt.md`).
   - Bump Grace's version (e.g. v1.19 → v1.20) + new changelog entry.
   - Run `scripts/update-receptionist-br-unified.js` to deploy.

3. **Follow-up requested by client (2026-05-08 PM):** *"would be great if Grace also passed CONTEXT to the destination on transfer"* — i.e. the receiver hears *"You have a caller on the line who's interested in a Builders Risk quote, here's their name…"* before being connected. This is VAPI's **warm transfer** feature — `transferPlan: { mode: 'warm-transfer-say-summary' }` or `'warm-transfer-say-message'` on the destination. Currently the proxy uses a plain blind transfer. Implementation work for next session — not a quick edit because it involves: (a) configuring `transferPlan` on each destination of `transfer_to_specific_person`, (b) deciding whether the summary is LLM-generated from the transcript or a templated message Grace builds. See [VAPI Call Forwarding docs](https://docs.vapi.ai/call-forwarding) §Warm Transfer.
