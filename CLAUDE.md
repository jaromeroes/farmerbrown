...
- **The Voice Lab lives in `billing/`** (`src/pages/portal/voices.astro`) but
  belongs to the voice-agent system — the customer auditions candidate agent
  voices there and sends a shortlist. Audio + catalogue are generated by
  `voice-agents/scripts/voice-lab-build.js`. Docs: `voice-agents/docs/voice-lab.md`.
