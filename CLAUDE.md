...
| `transfer_to_live_agent_builders_risk` | `7eb304a7-ee98-4076-be2f-2d1c5fd6645e` | transferCall | Transfer to +18775131573 (BuildersRisk.Net live-agent line). **WARM TRANSFER since 2026-06-11**: mode `warm-transfer-wait-for-operator-to-speak-first-and-then-say-summary` — the human hears a 2-sentence call summary before the caller is bridged. Shared by Jennifer AND `BR Live Agent Proxy v1.0` (both paths warm). Rollback: `node scripts/add-warm-transfer-to-br-live-agent-tool.js --rollback`. |
