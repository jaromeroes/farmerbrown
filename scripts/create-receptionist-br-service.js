...
// Grace Service receptionist — buildersrisk.net English Service line
// Mirror of Emma Service (FB) with BR branding + BR Live Agent Proxy destination.
...
async function createAssistant() {
  ...
  const res = await fetch('https://api.vapi.ai/assistant', {
    ...
    body: JSON.stringify({
      name: 'Grace — BR Receptionist EN Service v1.0',
  ...
  if (data.id) {
    console.log('Grace Service (BR) created successfully!');
    console.log('   Assistant ID:', data.id);
    console.log('   Name:', data.name);
    console.log('\nNext step: add this ID to scripts/create-squad-br-service.js and run it.');
  } else {
