...
// New Live Agent Proxy assistants (one per site)
...
// VAPI Names (must match exactly — receptionists will be bumped to v1.8 / v1.6 / v1.6 in the prompt-cleanup step)
const EMMA_NAME     = 'Emma — FB Receptionist EN Sales v1.8';
const OLIVIA_NAME   = 'Olivia — CL Receptionist EN Sales v1.6';
const GRACE_NAME    = 'Grace — BR Receptionist EN Sales v1.6';
...
// ────────────────────────────────────────────────────────────────────
// Shared L2 → L3 destinations (Emma + Olivia)
// ────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────
// NEW — Live agent proxy destinations (per site).
// This is the fix for the L2→L3 handoff bug: replacing the explicit
// transfer_to_live_agent_* tool with a squad destination of the same
// type as the specialists. The LLM now picks by name, no more tool bias.
// ────────────────────────────────────────────────────────────────────
...
const fbLiveAgentDestination = {
  ...
  description: "Transfer here when: caller has an EXISTING quote (hot lead / winner), caller is an existing policyholder on the wrong line, caller wants Workers' Compensation (no AI specialist yet), caller explicitly asks for a person, or fallback after two unclear routing attempts."
};

const clLiveAgentDestination = {
  ...fbLiveAgentDestination,
  assistantName: CL_PROXY_NAME
};

const brLiveAgentDestination = {
  ...fbLiveAgentDestination,
  assistantName: BR_PROXY_NAME
};

// ────────────────────────────────────────────────────────────────────
// Grace-specific variants (BR is the default path; alternate menu is Step 3)
// ────────────────────────────────────────────────────────────────────
...
// Receptionist version bumps — dispatcher must reference the new names.
// ────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────
// Destination sets per receptionist (4 specialists + site-specific proxy)
// ────────────────────────────────────────────────────────────────────

const emmaDestinations = [
  brDestination, glDestination, caDestination, haDestination,
  fbLiveAgentDestination
];
...
const graceDestinations = [
  brDestinationForGrace, glDestinationForGrace, caDestinationForGrace, haDestinationForGrace,
...
async function main() {
  console.log('Adding Live Agent Proxy destinations to all 4 squads...\n');
